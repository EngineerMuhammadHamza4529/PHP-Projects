...
$id=$_GET['eyeD'];
 ...
 //testing
 $q1="select * from testing where TestID='$id' ";
 $rows1=mysqli_query($con,$q1);
 $data1=mysqli_fetch_assoc($rows1);
 $dep=$data1['DepartmentID'];
?>
<body>
              ...
              <div class="card">
                <div class="card-body">
                  ...
                  <div class="form-group">
                    ...
                    <select name="dep" class="js-example-basic-single w-100">
                    <?php while ($data=mysqli_fetch_assoc($rows)) {?>
                      <option  value="<?php echo $data['DepartmentID']?>" <?php if($data['DepartmentID'] == $dep){ echo "selected"; } ?>><?php echo $data['DepartmentName']?></option>
                      <?php }?>
                    </select>    
                  </div>
                    <div class="form-group">
                      <label for="exampleInputName1">Test Name</label>
                      <input type="text" name="test_name" value="<?php echo $data1['TestName']; ?>" class="form-control" id="exampleInputName1" placeholder="Enter Test Name">
                    </div>
                    <input type="submit" name="test_up" class="btn btn-primary mr-2"/>
...
<?php
include('Connect.php');
if(isset($_POST['test_up']))
{
 $dept=$_POST['dep'];
 $tst=$_POST['test_name'];
  $q="Update testing SET DepartmentID='$dept',TestName='$tst' where TestID='$id' ;";
  $run=mysqli_query($con,$q);
    if($run){

        echo"<script> alert('Record is Succesfully Updated'); window.location.href='showtesting.php' </script>";
    }
